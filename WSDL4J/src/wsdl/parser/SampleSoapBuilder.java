...
public class SampleSoapBuilder implements MessageBuilder {
	private XmlMessageFormat format = new XmlMessageFormat();

	private final static Logger log = Logger.getLogger(SampleSoapBuilder.class);

	private WsdlContext wsdlContext;
	private WsdlInterface iface;
	private Map<QName, String[]> multiValues = null;

	public SampleSoapBuilder(WsdlInterface iface) throws Exception {
		this.iface = iface;
		this.wsdlContext = iface.getWsdlContext();
	}

	public SampleSoapBuilder(WsdlContext wsdlContext) {
	...
	public SoapVersion getSoapVersion() {
		return iface == null ? wsdlContext.getSoapVersion() : iface
				.getSoapVersion();
	}
	...
	public static String buildFault(String faultcode, String faultstring,
		...
		if (soapVersion == SoapVersion.Soap11) {
			emptyResponse = XmlUtils.setXPathContent(emptyResponse,
					"//faultcode", faultcode);
			emptyResponse = XmlUtils.setXPathContent(emptyResponse,
					"//faultstring", faultstring);
		} else if (soapVersion == SoapVersion.Soap12) {
			emptyResponse = XmlUtils.setXPathContent(emptyResponse,
					"//soap:Value", faultcode);
			emptyResponse = XmlUtils.setXPathContent(emptyResponse,
					"//soap:Text", faultstring);
			emptyResponse = XmlUtils.setXPathContent(emptyResponse,
	...
	private static String buildEmptyFault(SampleXmlUtil generator,
			SoapVersion soapVersion) {
		...
		try {
			...
			if (cursor.toChild(soapVersion.getEnvelopeQName())
					&& cursor.toChild(soapVersion.getBodyQName())) {
				SchemaType faultType = soapVersion.getFaultType();
				Node bodyNode = cursor.getDomNode();
				Document dom = XmlUtils.parseXml(generator
						.createSample(faultType));
				bodyNode.appendChild(bodyNode.getOwnerDocument().importNode(
						dom.getDocumentElement(), true));
			}
	...
	public String buildSoapMessageFromInput(BindingOperation bindingOperation,
			boolean buildOptional) throws Exception {
		return buildSoapMessageFromInput(bindingOperation, buildOptional, true);
	}

	/**
	 * Create sample SOAP Request for an WSDL operation
	 ...
	 */
	public String buildSoapMessageFromInput(BindingOperation bindingOperation,
			boolean buildOptional, boolean alwaysBuildHeaders) throws Exception {
		boolean inputSoapEncoded = WsdlUtils
				.isInputSoapEncoded(bindingOperation);
		ParserUtils xmlGenerator = new ParserUtils(inputSoapEncoded,
		...
		if (alwaysBuildHeaders) {
			...
			if (bindingInput != null) {
				List<?> extensibilityElements = bindingInput
						.getExtensibilityElements();
				List<SoapHeader> soapHeaders = WsdlUtils
						.getSoapHeaders(extensibilityElements);
	...
	private void addHeaders(List<SoapHeader> headers, XmlCursor cursor,
			ParserUtils xmlGenerator) throws Exception {
		// reposition
		cursor.toStartDoc();
		...
		for (int i = 0; i < headers.size(); i++) {
			...
			Message message = wsdlContext.getDefinition().getMessage(
					header.getMessage());
			if (message == null) {
	...
	/**
	 * Create element for a part of Input of a WSDL operation
	 * @param part
	 * @param cursor
	 * @param xmlGenerator
	 * @throws Exception
	 */
	public void createElementForPart(Part part, XmlCursor cursor,
			ParserUtils xmlGenerator) throws Exception {
		...
		if (elementName != null) {
			...
			if (wsdlContext.hasSchemaTypes()) {
				SchemaGlobalElement elm = wsdlContext.getSchemaTypeLoader()
						.findElement(elementName);
				if (elm != null) {
					this.format.complexTypes.add(elm);// TODO
					cursor.toFirstChild();
					this.format.merge(xmlGenerator.createSampleForType(elm
							.getName().getLocalPart(), elm.getType(), cursor));// TODO
				} else
					log.error("Could not find element [" + elementName
			...
			if (typeName != null && wsdlContext.hasSchemaTypes()) {
				SchemaType type = wsdlContext.getSchemaTypeLoader().findType(
						typeName);

				if (type != null) {
					cursor.toFirstChild();
					this.setFormat(xmlGenerator.createSampleForType(part.getName(),type,
							cursor));
				} else
	...
	private void buildDocumentRequest(BindingOperation bindingOperation,
			XmlCursor cursor, ParserUtils xmlGenerator) throws Exception {
	...
	private void buildDocumentResponse(BindingOperation bindingOperation,
			XmlCursor cursor, ParserUtils xmlGenerator) throws Exception {
	...
	private void buildRpcRequest(BindingOperation bindingOperation,
			XmlCursor cursor, ParserUtils xmlGenerator) throws Exception {
		// rpc requests use the operation name as root element
		String ns = WsdlUtils.getSoapBodyNamespace(bindingOperation
				.getBindingInput().getExtensibilityElements());
		if (ns == null) {
			ns = WsdlUtils.getTargetNamespace(wsdlContext.getDefinition());
			log
					.warn("missing namespace on soapbind:body for RPC request, using targetNamespace instead (BP violation)");
		}

		cursor.beginElement(new QName(ns, bindingOperation.getName()));
		if (xmlGenerator.isSoapEnc())
			cursor.insertAttributeWithValue(new QName(wsdlContext
					.getSoapVersion().getEnvelopeNamespace(), "encodingStyle"),
		...
		for (int i = 0; i < inputParts.length; i++) {
			...
			if (WsdlUtils.isAttachmentInputPart(part, bindingOperation)) {
				if (iface.getSettings().getBoolean(
					...
					c.insertAttributeWithValue("href", part.getName()
							+ "Attachment");
					c.dispose();
				}
			} else {
				if (wsdlContext.hasSchemaTypes()) {
					QName typeName = part.getTypeName();
					if (typeName != null) {
						SchemaType type = wsdlContext.getInterfaceDefinition()
								.findType(typeName);
						...
						if (type != null) {
							...
							this.setFormat(xmlGenerator.createSampleForType(part.getName(),
									type, c));
							c.dispose();
						} else
							log.warn("Failed to find type [" + typeName + "]");
					} else {
						SchemaGlobalElement element = wsdlContext
								.getSchemaTypeLoader().findElement(
										part.getElementName());
						if (element != null) {
							XmlCursor c = cursor.newCursor();
							c.toLastChild();
							...
							this.setFormat(xmlGenerator.createSampleForType(element.getName().getLocalPart(),
									element.getType(), c));
							c.dispose();
						} else
	...
	private void buildRpcResponse(BindingOperation bindingOperation,
			XmlCursor cursor, ParserUtils xmlGenerator) throws Exception {
		// rpc requests use the operation name as root element
		BindingOutput bindingOutput = bindingOperation.getBindingOutput();
		String ns = bindingOutput == null ? null : WsdlUtils
				.getSoapBodyNamespace(bindingOutput.getExtensibilityElements());

		if (ns == null) {
			ns = WsdlUtils.getTargetNamespace(wsdlContext.getDefinition());
			log
					.warn("missing namespace on soapbind:body for RPC response, using targetNamespace instead (BP violation)");
		}
	...
	public String buildSoapMessageFromOutput(BindingOperation bindingOperation,
			boolean buildOptional) throws Exception {
	...
	public String buildSoapMessageFromOutput(BindingOperation bindingOperation,
		...
		if (alwaysBuildHeaders) {
			...
			if (bindingOutput != null) {
				List<?> extensibilityElements = bindingOutput
						.getExtensibilityElements();
				List<SoapHeader> soapHeaders = WsdlUtils
						.getSoapHeaders(extensibilityElements);
				addHeaders(soapHeaders, cursor, xmlGenerator);
	...
	public String buildFault(FaultPart faultPart) {
		ParserUtils generator = new ParserUtils(false, null);// TODO 鏈夊彲鑳藉彂鐢熷紓甯�
		...
		try {
			...
			faultResponse = xmlObject.xmlText(new XmlOptions()
	...
	public void setFormat(XmlMessageFormat format) {
		this.format = format;
	}

	public XmlMessageFormat getFormat() {
		return format;
	}
